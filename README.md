...
After deploy / re-register, capture `username_link` from `phala logs` and set GitHub Actions variable `PUBLIC_SIGNAL_USERNAME_LINK` for the site Message button — [DEVELOPMENT.md — Signal username → site Message button](.agents/docs/DEVELOPMENT.md#signal-username-site-message-button).

...
impl TranslateHandler {
    ...
    async fn send_reply(
        ...
        if let Some(quote) = quote {
            ...
            let snippet = quote.text.as_deref().map(|t| truncate_snippet(t, 120));
    ...
    async fn translate_text(
        ...
        source: &str,
        lang: &Language,
    ) -> Result<String, near_ai_client::NearAiError> {
        let prompt = format!(
...
impl CommandHandler for TranslateHandler {
    ...
    async fn execute(&self, message: &BotMessage) -> AppResult<String> {
        ...
        let lang = match resolve_language(lang_token) {
            ...
            None => {
                let msg =
                    format!("Unknown language: {lang_token}. Use !list-langs for supported codes.");
                self.send_reply(message, None, &msg).await?;
                return Ok(String::new());
            }
        };
        ...
        let quote = match &message.quote {
            Some(q) => q,
            None => {
                let msg = "Reply to the message you want translated with: !translate <language>";
                self.send_reply(message, None, msg).await?;
                return Ok(String::new());
            }
        };
